...
public:
    ...
    pxr::HdSceneIndexBaseRefPtr inputScene() const {
        return m_inputScene;
    }
            ...
            if (auto dataSource =
                ...
                if (name == "bifrost:graph") {
                    if (value.IsHolding<pxr::TfToken>()) {
                        const auto& graphName  = value.UncheckedGet<pxr::TfToken>();
...
private:
    ...
    pxr::HdSceneIndexBaseRefPtr m_inputScene;
    Inputs      m_inputs;
    Output      m_output;
};
...
pxr::HdSceneIndexBaseRefPtr Parameters::inputScene() const {
    return m_impl->inputScene();
}
void Parameters::setInputScene(pxr::HdSceneIndexBaseRefPtr inputScene) {
    m_impl->setInputScene(std::move(inputScene));
}

void Parameters::setInputs(pxr::HdSceneIndexPrim const& prim) {
