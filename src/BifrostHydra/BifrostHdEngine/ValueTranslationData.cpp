...
Amino::Value ValueTranslationData::getInput(Amino::Type const& type) const {
    ...
    if (search != inputs.end()) {
        ...
        if (vtValue.IsHolding<bool>()) {
        ...
        } else if (vtValue.IsHolding<pxr::GfVec3f>()) {
            auto                  gf3 = vtValue.UncheckedGet<pxr::GfVec3f>();
            ...
            auto paths = vtValue.UncheckedGet<pxr::VtArray<pxr::SdfPath>>();
            if (paths.size() == 1) {
                auto sourceMeshPrim = inputScene->GetPrim(paths[0]);
                if (sourceMeshPrim.primType == pxr::HdPrimTypeTokens->mesh) {
